...
$viewer_type = $_SESSION['user_type'] ?? null;
...
$isOwner = $viewer_type === 'seller' && (int) $viewer_id === (int) $product['seller_id'];
$isAdmin = $viewer_type === 'admin';
$isPubliclyVisible = ($product['status'] ?? 'inactive') === 'active';
...
$reviews_enabled = (int) ($product['reviews_enabled'] ?? 1);
$review_error = '';

// Track product view for recommendations (best-effort).
track_product_event($pdo, $_SESSION['user_id'] ?? null, (int) $product_id, 'view', 1);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['rating']) && isset($_POST['comment']) && isset($_SESSION['user_id'])) {
    ...
    if (!$reviews_enabled) {
        $review_error = 'Reviews are disabled for this product.';
    } else {
    ...
    <div class="container my-5">
        <div class="row">
            <div class="col-md-6 position-relative">
                <?php if (isset($_SESSION['user_id']) && $_SESSION['user_type'] === 'seller' && $_SESSION['user_id'] == $product['seller_id']): ?>
                    <a href="seller/edit-product.php?id=<?= $product['id'] ?>"
                        ...
                        style="top:10px; right:10px; z-index:10;">Edit</a>
                <?php endif; ?>
                <img src="<?= htmlspecialchars($product['image']) ?>" class="product-detail-img"
            ...
            <div class="col-md-6">
                ...
                <?php else: ?>
                    <p class="text-muted mb-2">Reviews are disabled for this product.</p>
                <?php endif; ?>
                ...
                <?php if (!$isPubliclyVisible): ?>
                    <div class="alert alert-warning py-2">
                        This product is currently inactive and hidden from customers.
                    </div>
                <?php endif; ?>

                <?php if ($isPubliclyVisible && $product['stock'] > 0): ?>
                    ...
                    if ($viewer_id) {
                        ...
                        $wstmt->execute([$viewer_id, $product['id']]);
                        $inWishlist = (bool) $wstmt->fetch();
                    }
                    ?>
                    <button
                        class="btn btn-lg add-to-wishlist <?= $inWishlist ? 'btn-danger remove-mode' : 'btn-outline-danger' ?>"
                        data-id="<?= $product['id'] ?>">
        ...
        <div class="row mt-5">
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <small class="text-muted">Based on this item and recent activity</small>
                </div>
                <div class="row" data-ai-reco="1" data-product-id="<?= (int) $product['id'] ?>" data-limit="8">
            ...
            <?php if (isset($_SESSION['user_id'])): ?>
                <div class="row mt-4">
                    <div class="col-12">
                        ...
                        <?php if ($review_error): ?>
                            <div class="alert alert-warning"><?= htmlspecialchars($review_error) ?></div>
                        <?php endif; ?>
                        <form method="POST">
            ...
            <div class="row mt-5">
                <div class="col-12">
                    ...
                    <?php if ($review_error): ?>
                        <div class="alert alert-warning"><?= htmlspecialchars($review_error) ?></div>
                    <?php endif; ?>
                    <p class="text-muted">Reviews are disabled for this product.</p>
                </div>
            </div>
        <?php endif; ?>
    </div>
